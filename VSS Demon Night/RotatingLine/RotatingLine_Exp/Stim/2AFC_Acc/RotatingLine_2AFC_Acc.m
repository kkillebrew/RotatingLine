...
experiment = 'RotatingLine_2AFC_ACC';
...
elseif testComp == 1
    datadir = '/Users/gideon/Documents/Kyle/Rotating Line/Data/2AFC_Acc/';
end
...
rawdata2 = zeros(length(trialOrder2),7);  % CHANGE THIS
...
while 1
    [keyIsDown, secs, keycode] = KbCheck(dev_ID);
    if keycode(buttonY)
        rotLinePractice_2AFC_Acc(w,rect,dev_ID,x0,y0,tColor,buttonF,buttonJ,rotSpeed,hz,apTexture,gratingTexture,accRate,texX1,texY1,texX2,texY2,numPracTrials);
        break
...
for n=trialOrder(:,1)'
    ...
    elseif trialOrder(trialNumCounter,2) == 2
        ...
        for i=rawdata2(n,4:5)   % present the test or control
            ...
            [keyIsDown, secs, keycode] = KbCheck(dev_ID);
            while 1
                if (orientation2(trialNumCounter2,counter) > 0 && rotDir==1) || (orientation2(trialNumCounter2,counter) < -0 && rotDir==2)
                    [keyIsDown, secs, keycode] = KbCheck(dev_ID);
